...
describe('EDITABLE_QUESTION_TYPES', () => {
  test('excludes "mixed" (a generation-request modifier, never a real question type)', () => {
    expect(EDITABLE_QUESTION_TYPES.some((q) => q.value === 'mixed')).toBe(false);
  });

  test('contains all 6 real question types', () => {
...
describe('createEmptyQuestion', () => {
  test.each(['mcq', 'true_false', 'short_answer', 'descriptive', 'fill_blank', 'match'] as const)(
    'creates a valid-shape but empty %s question with a unique id',
    (type) => {
  ...
  test('mcq starts with 4 empty options and correctOptionIndex 0', () => {
    const q = createEmptyQuestion('mcq') as McqQuestion;
    expect(q.options).toEqual(['', '', '', '']);
    expect(q.correctOptionIndex).toBe(0);
  });

  test('match starts with 3 empty pairs (the minimum)', () => {
...
describe('fromWireQuestion / toWireQuestion round trip', () => {
  ...
  test('true_false normalizes correctAnswer to exactly "True"/"False"', () => {
  ...
  test('descriptive round-trips modelAnswer', () => {
  ...
  test('fill_blank round-trips correctAnswer and keeps the blank marker in text', () => {
  ...
  test('short_answer round-trips correctAnswer', () => {
  ...
  test('an unrecognized type falls back to short_answer rather than throwing', () => {
    const q = fromWireQuestion({ type: 'essay', text: 'Write about it.' });
    expect(q.type).toBe('short_answer');
  });

  test('a completely malformed input (not an object) never throws', () => {
  ...
  test('mcq with fewer than 4 raw options is padded, not truncated to invalid shape', () => {
    const q = fromWireQuestion({ type: 'mcq', text: 'Q', options: ['A', 'B'] }) as McqQuestion;
    expect(q.options).toHaveLength(4);
    expect(q.options.slice(0, 2)).toEqual(['A', 'B']);
  });

  test('an existing id is preserved by fromWireQuestion', () => {
    const q = fromWireQuestion({ id: 'stable-id-1', type: 'short_answer', text: 'Q', correctAnswer: 'A' });
    expect(q.id).toBe('stable-id-1');
  });

  test('toWireQuestion always includes every field (empty/-1 when not applicable), matching the server schema', () => {
...
describe('parseStructuredDocument', () => {
  ...
  test('returns null for a legacy structured payload with no schemaVersion (existing resources)', () => {
  ...
  test('returns null when schemaVersion is present but not 2', () => {
    expect(parseStructuredDocument(JSON.stringify({ schemaVersion: 1, questions: [] }))).toBeNull();
  });

  test('returns null when questions is missing or not an array', () => {
  ...
  test('parses a well-formed structured document, preserving generator-config keys', () => {
...
describe('buildStructuredPayload', () => {
  test('produces a JSON string parseStructuredDocument reads back identically', () => {
...
describe('validateQuestion', () => {
  test('rejects empty text for every type', () => {
    for (const type of ['mcq', 'true_false', 'short_answer', 'descriptive', 'fill_blank', 'match'] as const) {
      const q = createEmptyQuestion(type);
      expect(validateQuestion(q)).toBeTruthy();
    }
  });

  test('mcq: requires all 4 options filled', () => {
    const q = { ...createEmptyQuestion('mcq'), text: 'Q?', options: ['A', 'B', '', 'D'] } as McqQuestion;
    expect(validateQuestion(q)).toMatch(/all 4 options/i);
  });

  test('mcq: valid when text + all options filled + a correct index selected', () => {
  ...
  test('true_false: valid as soon as text is present (correctAnswer is always a valid toggle value)', () => {
    const q = { ...createEmptyQuestion('true_false'), text: 'The sky is blue.' };
    expect(validateQuestion(q)).toBeNull();
  });

  test('short_answer: requires a non-empty correctAnswer', () => {
  ...
  test('descriptive: requires a non-empty modelAnswer', () => {
  ...
  test('fill_blank: rejects text with no blank marker', () => {
  ...
  test('fill_blank: accepts a blank written with 3+ underscores and a correctAnswer', () => {
  ...
  test('match: rejects fewer than MIN_MATCH_PAIRS', () => {
  ...
  test('match: rejects more than MAX_MATCH_PAIRS', () => {
  ...
  test('match: rejects an empty side of a pair', () => {
    const q = {
  ...
  test('match: rejects duplicate left-hand values', () => {
    const q = {
      ...createEmptyQuestion('match'), text: 'Match.',
  ...
  test('match: valid with 3-8 unique, fully-filled pairs', () => {
    const q = {
      ...createEmptyQuestion('match'), text: 'Match.',
...
describe('validateQuestions', () => {
  test('returns an empty map when every question is valid', () => {
  ...
  test('keys errors by question id, only for invalid questions', () => {
