...
// `Question`/`StructuredAssessmentDocument` types in ../api/resources.
//
// Ported verbatim from client/src/lib/structuredQuestions.ts (same logic,
// only the import paths changed — see docs/generator-v2-plan.md's own "reuse
// the same structured-question contract" instruction). This is the mobile
// mirror of server/src/lib/assessmentSchema.js and the structured-question
// handling in server/src/routes/resources.js. It deliberately does NOT talk
// to the network — GeneratorFormScreen/GeneratorResultScreen/
// ResourceEditScreen own the API calls and use these functions to translate
// between the wire/storage shape (a flat object with every field present,
// empty when not applicable — the server's own convention) and a friendlier
// discriminated union for the editor UI.
//
 ...
 * `tryReadStructuredQuestions` (routes/resources.js) exactly, so mobile,
 * client, and server always agree on what counts as "this resource has
 * structured questions". A legacy resource (no schemaVersion, or a malformed/
 * missing `structured`) always returns null here — the caller falls back to
 * the flat markdown editor, unchanged (see docs/generator-v2-plan.md §5/§6).
 */
