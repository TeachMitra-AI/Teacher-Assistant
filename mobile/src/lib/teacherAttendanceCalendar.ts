// Calendar-day math for HistoryScreen's "show Absent days too" feature —
// ported verbatim from client/src/lib/teacherAttendanceCalendar.ts (pure, no
// DOM dependency; only the type import path differs).
import type { TeacherAttendanceDto, SchoolAttendanceConfigDto, SchoolHolidayDto } from '../types';

/** "YYYY-MM-DD" -> 0=Sunday..6=Saturday, using the device's local calendar. */
 ...
 * mirrors server/src/lib/teacherAttendance.js's isWeeklyOff() parsing
 * exactly (including the same "empty string means none configured" guard
 * against Number('') === 0).
 */
 ...
 * this specific person's own account was created. `undefined` when there's
 * no config yet.
 */
