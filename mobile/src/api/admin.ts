// Thin wrapper over the admin endpoints (server/src/routes/admin.js) — same
// role-scoped (own school / district / all) contract the web Admin
// Dashboard calls (client/src/lib/admin.ts), no backend change needed. Every
// listing here is paginated SERVER-side: the response carries `total` so the
// UI can render "showing 1–25 of 142" without ever holding all 142 rows.
// School scoping is enforced server-side from the auth token — none of
...
import type { Analytics, AdminSchool, AdminUser, Role, UserStatus } from '../types';
