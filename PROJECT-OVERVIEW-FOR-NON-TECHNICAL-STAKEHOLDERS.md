...
```mermaid
flowchart LR
    A["🧑‍🏫 A tool a few<br/>teachers use"] --> B["🎓 A standard companion<br/>every teacher reaches for"]
    B --> C["🏫 A clear picture for<br/>school & district leaders<br/>of what teachers need"]
    C --> D["🌍 A model other regions<br/>and countries can adopt"]
```
...
flowchart TD
    A[🔐 Teacher logs in] --> B[🧩 Opens the Generator]
    B --> C[📋 Picks Format, Subject &amp; Grade]
    C --> D[✍️ Types the Topic and sets Difficulty, Type &amp; Count]
    D --> E[✨ Clicks Generate — AI drafts the paper]
    E --> F[👀 Reviews, edits &amp; adds Paper Details]
    F --> G[💾 Saves to My Library]
    G --> H[🖨️ Prints and uses the paper in class]
```
...
```mermaid
flowchart TD
    S["🎓 Super Admin<br/>(entire system)"] --> D1["🏢 Resource Person<br/>(district / region)"]
    D1 --> A1["🏫 School Admin<br/>(one school)"]
    D1 --> A2["🏫 School Admin<br/>(one school)"]
    A1 --> T1["🧑‍🏫 Teacher"]
    A1 --> T2["🧑‍🏫 Teacher"]
    A2 --> T3["🧑‍🏫 Teacher"]
```
...
```mermaid
flowchart LR
    N["📍 Near-Term<br/>Lesson Plan Generator<br/>More languages<br/>Database scale-up"] --> M["📍 Medium-Term<br/>Teacher community space<br/>Video micro-learning<br/>Deeper analytics"]
    M --> L["📍 Long-Term<br/>State portal integration<br/>Certifications<br/>Multi-country expansion"]
```
